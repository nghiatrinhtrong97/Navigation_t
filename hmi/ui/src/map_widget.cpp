...
{
    // Draw start point - use square shape to differentiate from current position
    if (m_hasStartPoint) {
        QPoint startScreen = geoToScreen(m_startPoint);
        
        // Draw green square for start point
    ...
    // Draw end point - use triangle shape  
    if (m_hasEndPoint) {
        QPoint endScreen = geoToScreen(m_endPoint);
        
        // Draw red triangle for end point
        painter.setPen(QPen(Qt::darkRed, 3));
        painter.setBrush(QBrush(Qt::red));
        
        QPolygon triangle;
        triangle << QPoint(endScreen.x(), endScreen.y() - 12)        // Top
...
void MapWidget::drawCurrentPosition(QPainter& painter)
{
    ...
    // Draw large circle for current position with cross pattern
    painter.setPen(QPen(Qt::blue, 4));
    painter.setBrush(QBrush(QColor(30, 144, 255, 220))); // Bright blue
    painter.drawEllipse(posScreen, 16, 16);
    
    // Draw white cross inside to make it distinctive
    ...
    // Draw "C" text to identify Current Position
    painter.setPen(QPen(Qt::white, 2));
    painter.setFont(QFont("Arial", 10, QFont::Bold));
...
void MapWidget::drawClickedPoint(QPainter& painter)
{
    ...
    // Draw a larger, more visible clicked point marker
    painter.setPen(QPen(Qt::red, 4));
    painter.setBrush(QBrush(QColor(255, 0, 0, 200))); // Bright red
    ...
    // Draw cross to indicate clickable point
...
{
    // Always emit current mouse position coordinates
    Point mousePos = screenToGeo(event->pos());
    emit mousePositionChanged(mousePos);
